...
async function sendXAIResponse({ apiKey, model, input, maxOutputTokens = 800, temperature = 0.2, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  ...
  try {
    ...
    if (error && error.name === 'AbortError') {
      return { ok: false, error: { status: 408, message: 'xAI request timed out' } };
    }
